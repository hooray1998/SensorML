...
class unionUData():
    ...
    def getPosition(self, time):
        x = time*self.k1 + self.b1
        y = time*self.k2 + self.b2
        return x, y
    ...
    def updateValue(self):
        self.k1 = (self.d1[self.curIndex][1]-self.d1[self.curIndex-1][1])/(
                self.d1[self.curIndex][0]-self.d1[self.curIndex-1][0])
        self.k2 = (self.d1[self.curIndex][2]-self.d1[self.curIndex-1][2])/(
                self.d1[self.curIndex][0]-self.d1[self.curIndex-1][0])
        self.b1 = self.d1[self.curIndex][1] - self.k1*self.d1[self.curIndex][0]
        self.b2 = self.d1[self.curIndex][2] - self.k2*self.d1[self.curIndex][0]

    def union(self):

        for line in self.d2:
            while line[0] > self.d1[self.curIndex][0]:
                if self.curIndex < len(self.d1) - 1:
                    self.curIndex += 1
                    self.updateValue()
                else:
                    break
            x, y = self.getPosition(line[0])
            self.data.append(line + [x, y])
    ...
    def speed(self):
        ...
        for index in range(0, len(self.data), step):
            if index + step >= len(self.data):
                ...
                move_y = (self.data[index+step][3] - self.data[index][3]) * y2x
                curStep = []
